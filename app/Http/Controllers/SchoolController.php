...
class SchoolController extends Controller
{
    public function sregister(Request $request)
    {
        ...
        if ($request->hasFile('logo')) 
        {
            ...
            $file->move(public_path('uploads/schools/logos'), $logo_name);
            ...
            'logo' => 'uploads/schools/logos/'.$logo_name,
    ...
    public function createschoolbranch(Request $request, $id)
	{
        ...
        if($request->hasFile('branch_image'))
        {
            ...
            $file->move(public_path('uploads/schools/branchimages'), $filename1);
        }
        else
        {
        ...
        $inserted = SchoolBranch::create([
            ...
            'branch_image' => 'uploads/schools/branchimages/'.$filename1
            ]);
    ...
    public function createschoolevent(Request $request, $id)
	{
        ...
        if($validator->fails()){
			...
			if(!empty($request->hasFile('event_image')))
			{
                ...
                $file->move(public_path('uploads/schools/eventimages'), $filename1);
			}
			else
			{
				$filename1 = 'event.jpg';
			...
			if(!empty($request->hasFile('event_logo')))
			{
                ...
                $file->move(public_path('uploads/schools/eventlogos'), $logoname);
			}
			else
			{
            ...
            $inserted = SchoolEvent::create([
                ...
                'event_image1' => 'uploads/schools/eventimages/'.$filename1,
                'event_logo' => 'uploads/schools/eventlogos/'.$logoname,
    ...
    public function addschoolgallery(Request $request, $id)
    {
        ...
        if ($request->hasFile('gallery')) 
        {
            ...
            $file->move(public_path('uploads/schools/galleries'), $gallery_image_name);

            $gallery = new SchoolGallery();
            $gallery->school_id = $id;
            $gallery->user_id = auth()->user()->user_id;
            $gallery->gallery_id = 'SGA' . time();
            $gallery->image = 'uploads/schools/galleries/'.$gallery_image_name;
    ...
    public function schangecoverimage(Request $request, $school_id)
    {
        ...
        if ($request->hasFile('coverimage')) 
        {
            ...
            $file->move(public_path('uploads/schools/coverimages'), $coverimage_name);
            
            $data = [
            
                'cover_image' => 'uploads/schools/coverimages/'.$coverimage_name,
            ];

...
class AssociationController extends Controller
{
    ...
    public function aregister(Request $request)
        {
            ...
            if ($request->hasFile('logo')) 
            {
                ...
                $file->move(public_path('uploads/associations/logos'), $logo_name);
                ...
                'logo' => 'uploads/associations/logos/'.$logo_name,
    ...
    public function createassociationbranch(Request $request, $id)
	{
        ...
        if($request->hasFile('branch_image'))
        {
            ...
            $file->move(public_path('uploads/associations/branchimages'), $filename1);
        ...
        $inserted = AssociationBranch::create([
            ...
            'branch_image' => 'uploads/associations/branchimages/'.$filename1
            ]);
    ...
    public function createassociationevent(Request $request, $id)
	{
        ...
        if($validator->fails()){
			...
			if(!empty($request->hasFile('event_image')))
			{
                ...
                $file->move(public_path('uploads/associations/events/eventimages'), $filename1);
			}
			else
			{
				$filename1 = 'event.jpg';
			...
			if(!empty($request->hasFile('event_logo')))
			{
                ...
                $file->move(public_path('uploads/associations/events/eventlogos'), $logoname);
			}
			else
			{
			...
			'event_image1' => 'uploads/associations/events/eventimages/'.$filename1,
			'event_logo' => 'uploads/associations/events/eventlogos/'.$logoname,
    ...
    public function addassociationgallery(Request $request, $id)
    {
        ...
        if ($request->hasFile('gallery')) 
        {
            ...
            $file->move(public_path('uploads/associations/galleries'), $logoname);

            $gallery = new AssociationGallery();
            $gallery->association_id = $id;
            $gallery->user_id = auth()->user()->user_id;
            $gallery->gallery_id = 'AGA' . time();
            $gallery->image = 'uploads/associations/galleries/'.$gallery_image_name;
    ...
    public function achangelogo(Request $request, $association_id)
    {
        ...
        if ($request->hasFile('logo')) 
        {
            ...
            $logo_name = time().uniqid().'.'.$request->logo->extension();
            //$file->storeAs('associations/logos', $logo_name, 's3');
            $file->move(public_path('uploads/associations/logos'), $logoname);
    ...
    public function achangecoverimage(Request $request, $association_id)
    {
        ...
        if ($request->hasFile('coverimage')) 
        {
            ...
            $file->move(public_path('uploads/associations/coverimages'), $coverimage_name);
            
            $data = [
            
                'cover_image' => 'uploads/associations/coverimages/'.$coverimage_name,
            ];
